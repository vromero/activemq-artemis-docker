...
if [ -f /var/lib/artemis/etc-overwrite/custom-transformations.xslt ]; then
  echo Applying custom-transformations.xslt
  cp /var/lib/artemis/etc/broker.xml /tmp/broker.xml
  xmlstarlet tr /var/lib/artemis/etc-overwrite/custom-transformations.xslt /tmp/broker.xml > /var/lib/artemis/etc/broker.xml
else
  echo No custom-transformations.xslt found
fi

if [ -f /var/lib/artemis/etc-overwrite/broker.xml ]; then
  cp /var/lib/artemis/etc/broker.xml /tmp/broker.xml
  xmlstarlet tr /opt/merge/merge.xslt -s with=/var/lib/artemis/etc-overwrite/broker.xml /tmp/broker.xml > /var/lib/artemis/etc/broker.xml
else
