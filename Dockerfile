...
RUN cd /opt && wget -q http://www-us.apache.org/dist/activemq/activemq-artemis/1.5.1/apache-artemis-1.5.1-bin.tar.gz && \
  wget -q http://www.us.apache.org/dist/activemq/activemq-artemis/1.5.1/apache-artemis-1.5.1-bin.tar.gz.asc && \
  wget -q http://apache.org/dist/activemq/KEYS && \
  gpg --import KEYS && \
  gpg apache-artemis-1.5.1-bin.tar.gz.asc && \
  tar xfz apache-artemis-1.5.1-bin.tar.gz && \
  ln -s apache-artemis-1.5.1 apache-artemis && \
  rm -f apache-artemis-1.5.1-bin.tar.gz KEYS apache-artemis-1.5.1-bin.tar.gz.asc

# Create broker instance
RUN cd /var/lib && \
  /opt/apache-artemis-1.5.1/bin/artemis create artemis \
