...
RUN cd /opt && wget -q http://www-us.apache.org/dist/activemq/activemq-artemis/1.3.0/apache-artemis-1.3.0-bin.tar.gz && \
  ...
  tar xfz apache-artemis-1.3.0-bin.tar.gz && \
  ln -s apache-artemis-1.3.0 apache-artemis && \
  rm -f apache-artemis-1.3.0-bin.tar.gz KEYS apache-artemis-1.3.0-bin.tar.gz.asc

# Create broker instance
RUN cd /var/lib && \
  /opt/apache-artemis-1.3.0/bin/artemis create artemis \
