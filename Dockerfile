...
RUN cd /opt && wget -q https://archive.apache.org/dist/activemq/activemq-artemis/1.0.0/apache-artemis-1.0.0-bin.tar.gz && \
  wget -q https://archive.apache.org/dist/activemq/activemq-artemis/1.0.0/apache-artemis-1.0.0-bin.tar.gz.asc && \
  wget -q http://apache.org/dist/activemq/KEYS && \
  gpg --import KEYS && \
  gpg apache-artemis-1.0.0-bin.tar.gz.asc && \
  tar xfz apache-artemis-1.0.0-bin.tar.gz && \
  ln -s apache-artemis-1.0.0 apache-artemis && \
  rm -f apache-artemis-1.0.0-bin.tar.gz KEYS apache-artemis-1.0.0-bin.tar.gz.asc
